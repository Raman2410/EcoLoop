...
const BottomPanel = ({ pickups = [] }) => {
  const { pathname } = useLocation();

  // 🔹 Accurate eco impact (completed only)
  const completedPickups = pickups.filter(
    (p) => p.status?.toLowerCase() === "completed"
  );

  const totalKgRecycled = completedPickups.reduce((sum, pickup) => {
    const kg = LOAD_TO_KG[pickup.approxLoad] || 0;
    return sum + kg;
  }, 0);
  ...
  return (
    <div className="mt-14 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-100 rounded-2xl p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <a
        href={actionLink}
        className="inline-flex items-center justify-center px-6 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 transition text-sm font-medium"
      >
        {actionText}
      </a>
    </div>
