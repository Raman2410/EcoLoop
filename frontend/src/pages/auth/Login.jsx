...
useEffect(() => {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      if (status === 403) {
        toast.error(
          "Collector account not verified. Please complete OTP verification."
        );
      } else {
        toast.error(
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-6">
      <div className="w-full max-w-md bg-slate-900/60 backdrop-blur rounded-2xl p-8 border border-slate-800 shadow-xl">

        {/* Heading */}
        <h2 className="text-3xl font-serif text-slate-100 text-center mb-2">
          Welcome Back 👋
        </h2>
