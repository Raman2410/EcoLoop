...
// ✅ Removed useAuthContext entirely — no more useEffect redirect loop
const Register = () => {
  ...
  const submitHandler = async (e) => {
    ...
    if (
      role === "collector" &&
      (!businessName || !phone || !serviceArea || !vehicleType || !vehicleNumber)
    ) {
    ...
    try {
      ...
      if (role === "collector") {
        console.log("✅ Navigating to OTP page with userId:", res.userId);
        toast.success("OTP sent to your phone!");
        // Hard redirect — avoids any navigate() interference from other components
        window.location.href = `/collector/verify-otp/${res.userId}`;
      } else {
  ...
  const labelClass =
    "flex items-center gap-1.5 text-[11px] font-semibold text-slate-400 tracking-widest uppercase mb-1";
    ...
    <div className="h-screen flex overflow-hidden bg-slate-950">
      ...
      <div className="hidden lg:flex w-[42%] flex-shrink-0 relative overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-900 flex-col justify-center px-14 py-16 border-r border-slate-800/60">
        ...
        <div className="relative z-10">
          <h1 className="text-4xl font-serif text-slate-100 tracking-tight mb-3">
            EcoLoop
          </h1>
      ...
      {/* ── Right form panel ── */}
      <div className="flex-1 flex items-center justify-center px-8 bg-slate-950 overflow-hidden">
        <div className="w-full max-w-[440px]">
          ...
          <div className="mb-6">
            ...
            <p className="text-sm text-slate-400">
              {role === "collector"
                ? "Set up your collection business profile"
          ...
          <form onSubmit={submitHandler} className="flex flex-col gap-3">
            <div className={`grid gap-3 ${role === "collector" ? "grid-cols-2" : "grid-cols-1"}`}>
              <div>
